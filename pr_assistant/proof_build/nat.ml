...
let rec replace_nat x t f= match f with
  ...
  |Var_nat c -> if c=x then t else Var_nat c

let replace_comp x t f = match f with
  |Equal(a,b) -> Equal(replace_nat x t a,replace_nat x t b)
  |Not_equal(a,b) -> Not_equal(replace_nat x t a,replace_nat x t b)
  |Less_than(a,b) -> Less_than(replace_nat x t a,replace_nat x t b)
  |Less_than_strict(a,b) -> Less_than_strict(replace_nat x t a,replace_nat x t b)
  |More_than(a,b) -> More_than(replace_nat x t a,replace_nat x t b)
  |More_than_strict(a,b) ->  More_than_strict(replace_nat x t a,replace_nat x t b)
...
let is_comp_equiv c1 c2 = match c1,c2 with
